    ...
    value.forEach(function(val){
      for(let[key1, value1] of Object.entries(val)){  
        if(value1 == "integer" || value1 == "smallint" || value1 == 'bigint'){
          ...
          nieWiem={id: key+'.'+key1, label: key1, type: 'integer', optgroup: key,
          validation: {min: 0}};
          if(key1== 'schizophrenia'){nieWiem['input']='select';nieWiem['values'] = Schizo;}
          else if(key1== 'approximate_income'){nieWiem['input']='select';nieWiem['values'] = Approxi;}
          else if(key1== 'ethnic_category'){nieWiem['input']='select';nieWiem['values'] = Ethnic;}
          else if(key1== ''||key1== 'father_ethnicity_1'||key1== 'father_ethnicity_2'||key1== 'father_ethnicity_3'||key1== 'father_ethnicity_4'||key1== 'mother_ethnicity_1'||key1== 'mother_ethnicity_2'||key1== 'mother_ethnicity_3'||key1== 'mother_ethnicity_4'){nieWiem['input']='select';nieWiem['values'] = Ethnicity;}
          else if(key1== 'marital_status'){nieWiem['input']='select';nieWiem['values'] = Marital;}
          else if(key1== 'military_discharge'){nieWiem['input']='select';nieWiem['values'] = Military_dis;}
          else if(key1== 'military_rejected'){nieWiem['input']='select';nieWiem['values'] = Military_rej;}
          else if(key1== 'religion'){nieWiem['input']='select';nieWiem['values'] = Religion;}
          else if(key1== 'reported_race'){nieWiem['input']='select';nieWiem['values'] = Race}
          else if(key1== 'smoked_cigarettes'){nieWiem['input']='select';nieWiem['values'] = Smoked;}
        }
        else if(value1 == "real"){
          colTypeObj[key+'.'+key1]='double';
          nieWiem={id: key+'.'+key1, label: key1, type: 'double', optgroup: key,
          validation: {min: 0, step: 0.01}};
        }
        else if(value1 == "text" || value1 == 'character'){
          colTypeObj[key+'.'+key1]='string';
          nieWiem={id: key+'.'+key1, label: key1, type: 'string', optgroup: key};
        }
        else if(value1 == "date"){
          ...
          nieWiem= {id: key+'.'+key1, label: key1, type: 'boolean', optgroup: key,
            input: 'radio',
